...
function jBox(type, options) {
	...
	this.options = {
		...
		adjustDistance: 20,			// How far from the window edge we start adjusting, use an object to set different values: {bottom: 5, top: 50, left: 5, right: 20}
		...
		// Animations
		fade: 0,					// Fade duration in ms, set to 0 or false to disable
		...
		zIndex: 90000,				// Use a high zIndex (your overlay will have the lowest zIndex of all your jBoxes (with overlays) minus one)
	...
	var zIndexMaxModal = this.options.zIndex;
	
	// Create jBox
	this._create = function() {
		if (this.wrapper) return;
		//alert(jBox._getUniqueID());
		if( jQuery('.jBox-Modal').length > 1 ){
			zIndexMaxModal = parseInt(jQuery('.jBox-Modal:last').css('z-index')) + 1;
			//alert( zIndexMaxModal );
		}
		
		//var zIndexMaxModal = parseInt(jQuery('.jBox-Modal:last').css( "z-index" ));
		//if(zIndexMaxModal)
		//	zIndexMaxModal = jQuery('.jBox-Modal:last').css( "z-index" ) + 1;
		//alert( zIndexMaxModal );
	...
	this._addOverlay = function() {
		...
		if (!this.overlay) {
			...
			this.overlay = jQuery('<div/>', {id: 'jBox-overlay'}).css({display: 'none', opacity: 0, zIndex: (zIndexMaxModal - 1)}).appendTo(jQuery('body'));
			...
			/*if( jQuery('.jBox-Modal').length == 1 ){
				...
				alert( jQuery('.jBox-Modal:last').css( "zIndex" ) );
				//this.overlay = (jQuery('<div/>', {id: 'jBox-overlay'}).css({display: 'none', opacity: 0, zIndex: (jQuery('.jBox-Modal:last').css( "zIndex" ) + 1)}).appendTo(jQuery('body')));
			}*/
	...
	this._generateCSS = function() {
		...
		var animations = {
			...
			slideOpen: {
				...
				timing: 'ease-in',
				positions: {top: {'0%': -100}, right: {'0%': 500}, bottom: {'0%': 100}, left: {'0%': -500}},
				css: [['0%', 'translate%XY(%Vpx)'], ['100%', 'translate%XY(0px)']]
			},
			slideClose: {
				duration: (this.options.fade || 300),
				timing: 'ease-in-out',
				positions: {top: {'100%': -500}, right: {'100%': 500}, bottom: {'100%': 500}, left: {'100%': -500}},
				css: [['0%', 'translate%XY(0px)'], ['100%', 'translate%XY(%Vpx)']]
			},
			flipOpen: {
				duration: (this.options.fade || 300),
			...
			tada: {
				duration: (this.options.fade || 500),
...
jBox.prototype.audio = function(options) {
	...
	if (!jBox._audio[options.url]) {
		...
		//#{resource['plugins/jbox/audio/"+som+"']}
		jQuery('<source/>', {src: options.url + '.mp3.xhtml?ln=plugins'}).appendTo(audio);
		jQuery('<source/>', {src: options.url + '.ogg.xhtml?ln=plugins'}).appendTo(audio);
