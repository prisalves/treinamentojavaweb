(function($){
		...
		navRight.mmenu({
			...
			searchfield	: {
				...
				search				: true,
				showLinksOnly		: false,
				noResults: "Desculpe, item no menu não encontrado.",
		...
		$(".nav-mini").on('click',function(){
			...
			if( !body.hasClass("in") && $(window).width() < 940 ){
		...
		//////////NAV AND MAIN  COLLAPSE     //////////
		 /*$("a.nav-collapse").on('click',function(){
			 //alert($(window).width());
			 var body=$("body");
			 if( (body.hasClass("in") && $(window).width() > 768) || (!body.hasClass("in") && $(window).width() > 940 ) ){
				var body=$("body");
					body.toggleClass( "nav-collapse" );
				//$('.list').animate({left:"+="+widthOfHidden()+"px"},'slow',function(){ });
			 }else{
		 ...
		 $(window).resize(function(e) {
			 	if(evento){
						...
						/*if( body.hasClass("nav-collapse") && $(window).width() > 940){
							body.toggleClass( "nav-collapse" );
...
/*		$(".panel-tools[data-toolscolor]").each(function(i) {
		...
		jQuery(window).load(function() { // LOAD CSS,IMGs
			...
			//$(".modal").click(function(e){
				/*$(this).find(".modal-body").click(function(e){
					e.preventDefault();
					return false;
				});*/
				//$(".noui").removeClass("ui-button");
				//$(this).find(".modal-dialog").actuate('shake',null,0);
				//e.preventDefault();
				//return false;
			//});
